...
function openRoom(room) {
  ...
  if (room.status === "open") {
    ...
    alert(`${room.room} — ${room.name} is coming soon 🚀`);
  }
}
...
function renderRooms() {
    ...
    return `
      <div class="room-card"
        style="
          ...
          cursor:pointer;
        "
        data-room='${encodeURIComponent(JSON.stringify(r))}'>
  ...
  document.querySelectorAll(".room-card").forEach(function (card) {
    card.addEventListener("click", function () {
      try {
        var room = JSON.parse(decodeURIComponent(this.getAttribute("data-room")));
        openRoom(room);
      } catch (e) {
